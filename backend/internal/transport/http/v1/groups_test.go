...
func TestIntegration_GetGroups(t *testing.T) {
	...
	t.Run("Should return groups when player requirements are provided", func(t *testing.T) {
		mockGroupService.EXPECT().GetGroups(gomock.Any(), gomock.Any()).Return(
			[]repository.GroupWithPlayers{
				{
					GroupDTO: repository.GroupDTO{
						ID:          "AAAA",
						CommunityID: 1,
		...
		req := httptest.NewRequest(http.MethodGet, "/api/v1/groups", test.GetBody(map[string]interface{}{
			"platform":  "pc",
			"role":      "vanguard",
			"rankId":    "d3",
			"voiceChat": true,
	...
	t.Run("Should handle empty player requirements", func(t *testing.T) {
		mockGroupService.EXPECT().GetGroups(gomock.Any(), gomock.Any()).Return(
			[]repository.GroupWithPlayers{},
			int32(0),
			nil,
	...
	t.Run("Should return 400 on invalid player requirements", func(t *testing.T) {
		req := httptest.NewRequest(http.MethodGet, "/api/v1/groups", test.GetBody(map[string]interface{}{
			"platform": "invalid",
			"role":     "invalid",
			"rankId":   "invalid",
